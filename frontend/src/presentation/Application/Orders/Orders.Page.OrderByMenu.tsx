...
export default function OrderByMenu() {
    ...
    const options = [
        ...
        { value: "total asc", label: "Total - Lowest to Highest" },
        { value: "total desc", label: "Total - Highest to Lowest" },
    ];
    ...
    return (
        <PolymorphicAbstractTooltipDefaultPanel className="z-10 fixed mt-1">
            <RadioButtonMenu
                name="orderBy"
                screenReaderDescription="Choose how to sort the order list. Selection will apply immediately."
                options={options}
                rulingValue={orderBy}
                onChange={onChange}
                onClose={onClose}
