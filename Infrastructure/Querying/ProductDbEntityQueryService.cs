...
public class ProductDbEntityQueryService
{
    private readonly List<Func<List<ProductDbEntity>, List<ProductDbEntity>>> _inMemoryCallbacks = [];
    private readonly IDatabaseProviderSingleton _databaseProvider;
    private readonly Dictionary<string, Expression<Func<ProductDbEntity, object>>> _fieldMapping = new()
    {
        { "Price", p => p.Price },
        { "DateCreated", p => p.DateCreated },
    };

    private IQueryable<ProductDbEntity> _query;


    public ProductDbEntityQueryService(IDatabaseProviderSingleton databaseProvider, IQueryable<ProductDbEntity> query)
    {
    ...
    public async Task<List<ProductDbEntity>> ReturnResult()
    {
